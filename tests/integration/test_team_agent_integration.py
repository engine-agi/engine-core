...
async def test_team_agent_integration():
    ...
    assert (
        hierarchical_team.coordination_strategy
        == TeamCoordinationStrategy.HIERARCHICAL.value
    )
    ...
    assert (
        collaborative_team.coordination_strategy
    ...
    parallel_team = (
        ...
        .add_member(
            "dev_agent_1", TeamMemberRole.MEMBER, ["processing"], max_concurrent_tasks=3
        )
        .add_member(
            "dev_agent_2", TeamMemberRole.MEMBER, ["processing"], max_concurrent_tasks=3
        )
        .build(agents)
    )
    ...
    assert (
        parallel_team.coordination_strategy == TeamCoordinationStrategy.PARALLEL.value
    )
...
async def test_team_task_execution():
    ...
    context = TeamExecutionContext(project_id="test_project", user_id="test_user")
...
async def main():
    ...
    try:
        ...
        await test_team_agent_integration()
