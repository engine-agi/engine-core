"""
Observability service for monitoring and metrics collection.
"""
import logging
import uuid
...
class ObservabilityRepository(ABC):
    ...
    async def update_metrics(
        self, project_id: str, metrics_type: str, data: Dict[str, Any]
    ) -> None:
        """Update metrics data."""
        pass
...
class MockObservabilityRepository(ObservabilityRepository):
    ...
    async def get_logs(self, project_id: str, filter_criteria: LogFilter) -> LogResult:
        ...
        for log in logs:
            if filter_criteria.level and log.get("level") != filter_criteria.level:
                continue
            if (
                filter_criteria.entity_type
                and log.get("entity_type") != filter_criteria.entity_type
            ):
                continue
            if (
                filter_criteria.entity_id
                and log.get("entity_id") != filter_criteria.entity_id
            ):
                continue
            ...
            if timestamp_str is not None:
                if (
                    ...
                    and timestamp_str < filter_criteria.start_time.isoformat()
                ):
                    continue
                if (
                    filter_criteria.end_time
                    and timestamp_str > filter_criteria.end_time.isoformat()
                ):
                    continue
        ...
        return LogResult(logs=paginated_logs, total=total, has_more=end_idx < total)
    ...
    async def get_project_health(self, project_id: str) -> ProjectHealthData:
        ...
        return ProjectHealthData(
            ...
            components=ComponentsHealth(
                agents=ComponentHealth(
                    status="healthy", healthy_count=4, total_count=5, error_rate=0.05
                ),
                teams=ComponentHealth(
                    status="healthy", healthy_count=2, total_count=2, error_rate=0.0
                ),
                workflows=ComponentHealth(
                    status="degraded", healthy_count=8, total_count=10, error_rate=0.15
                ),
                tools=ComponentHealth(
                    status="healthy", healthy_count=15, total_count=15, error_rate=0.02
                ),
            ),
            error_rate=0.08,
            availability=0.95,
...
class ObservabilityService:
    ...
    async def get_project_metrics(self, project_id: str) -> ProjectMetricsData:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get project metrics for {project_id}: {str(e)}"
            )
    ...
    async def get_performance_metrics(self, project_id: str) -> PerformanceMetricsData:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get performance metrics for {project_id}: {str(e)}"
            )
    ...
    async def get_project_health(self, project_id: str) -> ProjectHealthData:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get project health for {project_id}: {str(e)}"
            )
    ...
    async def get_component_logs(
        self, project_id: str, component_type: str, component_id: str, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_error_summary(
        ...
        try:
            ...
            for log in error_logs.logs:
                ...
                error_by_component[component_key] = (
                    error_by_component.get(component_key, 0) + 1
                )
            ...
            self.logger.error(
                f"Failed to get error summary for project {project_id}: {str(e)}"
            )
