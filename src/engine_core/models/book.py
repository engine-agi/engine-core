...
class Book(SQLAlchemyBaseModel):
    ...
    description = Column(Text, nullable=True, comment="Book description and purpose")
    ...
    search_config = Column(
        JSONB, nullable=True, comment="Semantic search configuration and settings"
    )

    # Book metadata
    author = Column(String(255), nullable=True, comment="Book author or creator")
    ...
    chapter_count = Column(
        Integer, default=0, nullable=False, comment="Number of chapters in book"
    )

    page_count = Column(
        Integer, default=0, nullable=False, comment="Total number of pages in book"
    )
    ...
    last_accessed_at = Column(
        String(50), nullable=True, comment="Last access timestamp"
    )

    # Collaborative features
    contributors = Column(
        ARRAY(String(255)), nullable=True, comment="List of contributors to the book"
    )

    # Book-specific metadata (avoiding conflict with SQLAlchemy's reserved 'metadata' attribute)
    book_metadata = Column(
        JSONB, nullable=True, comment="Book-specific metadata and configuration"
    )
    ...
    def add_chapter(
        self, chapter_id: str, title: str, description: Optional[str] = None
    ) -> "BookChapter":
        """Add chapter to book."""
        chapter = BookChapter(
            id=chapter_id, book_id=self.id, title=title, description=description
        )
    ...
    def is_semantic_search_enabled(self) -> bool:
        ...
        search_config_value = getattr(self, "search_config", None)
        return search_config_value is not None and search_config_value.get(
            "enabled", False
        )
    ...
    def record_access(self, accessor: Optional[str] = None) -> None:
        ...
        access_history.append(
            {"timestamp": datetime.utcnow().isoformat(), "accessor": accessor}
        )
...
class BookChapter(SQLAlchemyBaseModel):
    ...
    # Chapter information
    title = Column(String(255), nullable=False, comment="Chapter title")

    description = Column(
        Text, nullable=True, comment="Chapter description and overview"
    )
    ...
    # Chapter ordering
    order_index = Column(Integer, nullable=True, comment="Chapter order within book")

    # Content statistics
    page_count = Column(
        Integer, default=0, nullable=False, comment="Number of pages in chapter"
    )

    # Chapter metadata
    tags = Column(ARRAY(String(100)), nullable=True, comment="Chapter-specific tags")

    # Chapter-specific metadata (avoiding conflict with SQLAlchemy's reserved 'metadata' attribute)
    chapter_metadata = Column(
        JSONB, nullable=True, comment="Chapter-specific metadata and configuration"
    )
    ...
    pages = relationship(
        "BookPage", back_populates="chapter", cascade="all, delete-orphan"
    )
...
class BookPage(SQLAlchemyBaseModel):
    ...
    # Page information
    title = Column(String(255), nullable=False, comment="Page title")
    ...
    # Page ordering
    order_index = Column(Integer, nullable=True, comment="Page order within chapter")
    ...
    word_count = Column(Integer, nullable=True, comment="Approximate word count")
    ...
    # Page versioning
    version = Column(String(50), nullable=False, default="1.0", comment="Page version")
    ...
    # Page tags
    tags = Column(ARRAY(String(100)), nullable=True, comment="Page-specific tags")

    # Page-specific metadata (avoiding conflict with SQLAlchemy's reserved 'metadata' attribute)
    page_metadata = Column(
        JSONB, nullable=True, comment="Page-specific metadata and configuration"
    )
...
CheckConstraint(
    Book.status.in_(
        [BookStatus.ACTIVE, BookStatus.ARCHIVED, BookStatus.PRIVATE, BookStatus.SHARED]
    ),
    name="ck_book_status_valid",
)

CheckConstraint(Book.chapter_count >= 0, name="ck_book_chapter_count_non_negative")

CheckConstraint(Book.page_count >= 0, name="ck_book_page_count_non_negative")

CheckConstraint(Book.access_count >= 0, name="ck_book_access_count_non_negative")

CheckConstraint(BookChapter.page_count >= 0, name="ck_chapter_page_count_non_negative")
...
CheckConstraint(BookPage.access_count >= 0, name="ck_page_access_count_non_negative")
