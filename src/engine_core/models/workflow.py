...
class Workflow(SQLAlchemyBaseModel):
    ...
    def update_graph_metrics(self) -> None:
        ...
        try:
            ...
            depth = nx.dag_longest_path_length(
                G) if nx.is_directed_acyclic_graph(G) else 0

            # Graph width (maximum vertices at same level)
            levels = {}
            if nx.is_directed_acyclic_graph(G):
                for node in nx.topological_sort(G):
                    level = max([levels.get(pred, -1)
                                for pred in G.predecessors(node)], default=-1) + 1
                    levels[node] = level
            ...
            complexity = len(self.edges) / \
                             len(self.vertices) if len(self.vertices) > 0 else 0
            ...
            parallelism_potential = 1 - \
                (depth / len(self.vertices)) if len(self.vertices) > 0 else 0
    ...
    def get_execution_order(self) -> List[List[str]]:
        ...
        execution_mode = getattr(self, 'execution_mode',
                                 WorkflowExecutionMode.SEQUENTIAL.value)
    ...
    def update_execution_stats(self, execution_time: float, success: bool = True) -> None:
        ...
        if self.average_execution_time is None:
            ...
            total_time = self.average_execution_time * \
                (self.total_executions - 1) + execution_time
