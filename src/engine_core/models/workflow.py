...
# Optional import for graph validation
try:
    import networkx as nx  # type: ignore  # For graph validation
except ImportError:
    nx = None
...
class Workflow(SQLAlchemyBaseModel):
    """
    ...
    name = Column(
        String(255), nullable=False, index=True, comment="Human-readable workflow name"
    )

    description = Column(
        Text, nullable=True, comment="Workflow purpose, steps, and expected outcomes"
    )
    ...
    timeout = Column(Integer, nullable=True, comment="Workflow timeout in seconds")
    ...
    vertex_count = Column(
        Integer, default=0, nullable=False, comment="Number of vertices in workflow"
    )

    edge_count = Column(
        Integer, default=0, nullable=False, comment="Number of edges in workflow"
    )
    ...
    successful_executions = Column(
        Integer, default=0, nullable=False, comment="Number of successful executions"
    )
    ...
    workflow_metadata = Column(
        JSONB, nullable=True, comment="Workflow-specific metadata and configuration"
    )
    ...
    vertices = relationship(
        "WorkflowVertex", back_populates="workflow", cascade="all, delete-orphan"
    )
    edges = relationship(
    ...
    def validate_max_parallel_vertices(
        ...
        if value is not None:
            if not isinstance(value, int) or value < 1 or value > 100:
                raise ValueError(
                    "Max parallel vertices must be an integer between 1 and 100"
                )
        return value
    ...
    def validate_dag(self) -> tuple[bool, List[str]]:
        ...
        if nx is None:
            return True, ["NetworkX not available for graph validation"]
        ...
        if len(self.vertices) > 1:
            isolated = []
            for v in G.nodes():
                degree = len(list(G.neighbors(v))) + len(list(G.predecessors(v)))
                if degree == 0:
                    isolated.append(v)
            if isolated:
        ...
        if not sinks:
            errors.append(
                "Workflow must have at least one sink vertex (no outgoing edges)"
            )
        ...
        if not sources:
            errors.append(
                "Workflow must have at least one source vertex (no incoming edges)"
            )
    ...
    def update_graph_metrics(self) -> None:
        ...
        try:
            ...
            depth = (
                nx.dag_longest_path_length(G) if nx and nx.is_directed_acyclic_graph(G) else 0
            )

            # Graph width (maximum vertices at same level)
            levels = {}
            if nx and nx.is_directed_acyclic_graph(G):
                for node in nx.topological_sort(G):
                    level = (
                        max(
                            [levels.get(pred, -1) for pred in G.predecessors(node)],
                            default=-1,
                        )
                        + 1
                    )
            ...
            complexity = (
                len(self.edges) / len(self.vertices) if len(self.vertices) > 0 else 0
            )
            ...
            parallelism_potential = (
                1 - (depth / len(self.vertices)) if len(self.vertices) > 0 else 0
            )
            ...
            self.graph_metrics = {
                ...
                "is_dag": nx.is_directed_acyclic_graph(G) if nx else True,
    ...
    def get_execution_order(self) -> List[List[str]]:
        ...
        execution_mode = getattr(
            self, "execution_mode", WorkflowExecutionMode.SEQUENTIAL.value
        )

        if execution_mode == WorkflowExecutionMode.SEQUENTIAL.value:
            # Sequential execution - topological sort
            if nx is None:
                return [[vertex.id for vertex in self.vertices]]  # Fallback without networkx
    ...
    def add_edge(self, source_id: str, target_id: str, **edge_data) -> "WorkflowEdge":
        ...
        edge = WorkflowEdge(
            workflow_id=self.id, source_id=source_id, target_id=target_id, **edge_data
        )
        # In real implementation, this would add to session
    ...
    def update_execution_stats(
        self, execution_time: float, success: bool = True
    ) -> None:
...
class WorkflowVertex(SQLAlchemyBaseModel):
    """
    ...
    name = Column(String(255), nullable=False, comment="Human-readable vertex name")

    description = Column(
        Text, nullable=True, comment="Vertex purpose and functionality"
    )
    ...
    estimated_duration = Column(
        Integer, nullable=True, comment="Estimated execution duration in seconds"
    )

    timeout = Column(Integer, nullable=True, comment="Vertex timeout in seconds")
    ...
    input_channels = Column(
        ARRAY(String(100)), nullable=True, comment="Input channels this vertex accepts"
    )
    ...
    messages = Column(
        JSONB, nullable=True, comment="Messages received/sent in current superstep"
    )

    # Execution results and metadata
    execution_data = Column(
        JSONB, nullable=True, comment="Execution results and intermediate data"
    )

    # Vertex-specific metadata (avoiding conflict with SQLAlchemy's reserved 'metadata' attribute)
    vertex_metadata = Column(
        JSONB, nullable=True, comment="Vertex-specific metadata and configuration"
    )
    ...
    def validate_status(self, key: str, value: str) -> str:
        ...
        if value not in valid_statuses:
            raise ValueError(
                f"Vertex status must be one of: {', '.join(valid_statuses)}"
            )

        return value


class WorkflowEdge(SQLAlchemyBaseModel):
    """
    ...
    # Edge endpoints
    source_id = Column(
        String(255), nullable=False, index=True, comment="Source vertex ID"
    )

    target_id = Column(
        String(255), nullable=False, index=True, comment="Target vertex ID"
    )
    ...
    # Edge properties
    condition = Column(
        String(500), nullable=True, comment="Conditional expression for edge traversal"
    )

    weight = Column(
        Float, default=1.0, nullable=False, comment="Edge weight for graph algorithms"
    )
    ...
    edge_metadata = Column(
        JSONB, nullable=True, comment="Edge-specific metadata and configuration"
    )
...
# Unique constraints
UniqueConstraint(
    WorkflowVertex.workflow_id, WorkflowVertex.id, name="uq_vertex_workflow_id"
)

UniqueConstraint(
    WorkflowEdge.workflow_id,
    WorkflowEdge.source_id,
    WorkflowEdge.target_id,
...
CheckConstraint(
    Workflow.vertex_count >= 0, name="ck_workflow_vertex_count_non_negative"
)

CheckConstraint(Workflow.edge_count >= 0, name="ck_workflow_edge_count_non_negative")
...
class WorkflowExecution(SQLAlchemyBaseModel):
    """
    ...
    workflow_version = Column(
        String(50), nullable=True, comment="Version of workflow at execution time"
    )

    workflow_name = Column(
        String(255), nullable=True, comment="Name of workflow at execution time"
    )
    ...
    duration_seconds = Column(
        Float, nullable=True, comment="Total execution duration in seconds"
    )

    # Execution results
    success = Column(Boolean, nullable=True, comment="Whether execution was successful")
    ...
    input_data = Column(JSONB, nullable=True, comment="Input data provided to workflow")

    output_data = Column(
        JSONB, nullable=True, comment="Output data produced by workflow"
    )

    execution_context = Column(
        JSONB, nullable=True, comment="Execution context and environment details"
    )

    # Vertex execution tracking
    vertex_results = Column(
        JSONB, nullable=True, comment="Results from individual vertex executions"
    )
    ...
    total_vertices = Column(
        Integer, nullable=True, comment="Total number of vertices executed"
    )

    completed_vertices = Column(
        Integer, nullable=True, comment="Number of vertices that completed successfully"
    )

    failed_vertices = Column(
        Integer, nullable=True, comment="Number of vertices that failed"
    )

    # Resource usage
    resource_usage = Column(
        JSONB, nullable=True, comment="Resource usage statistics (CPU, memory, etc.)"
    )
    ...
    timeout_seconds = Column(
        Integer, nullable=True, comment="Configured timeout for execution"
    )
    ...
    execution_metadata = Column(
        JSONB, nullable=True, comment="Additional execution metadata"
    )
...
try:
    ...
    Project.workflow_executions = relationship(
        "WorkflowExecution", back_populates="project"
    )
except ImportError:
...
CheckConstraint(WorkflowExecution.priority >= -10, name="ck_execution_priority_range")

CheckConstraint(
    WorkflowExecution.priority <= 10, name="ck_execution_priority_range_upper"
)
