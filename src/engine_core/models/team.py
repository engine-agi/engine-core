...
team_agents = Table(
    ...
    Column(
        "joined_at", String(50), nullable=True, comment="When agent joined the team"
    ),
    Column(
        "is_active", Boolean, default=True, comment="Whether agent is active in team"
    ),
)
...
class Team(SQLAlchemyBaseModel):
    ...
    name = Column(
        String(255), nullable=False, index=True, comment="Human-readable team name"
    )

    description = Column(
        Text, nullable=True, comment="Team purpose, objectives, and working methods"
    )
    ...
    total_executions = Column(
        Integer, default=0, nullable=False, comment="Total number of team executions"
    )
    ...
    current_workflow = Column(
        String(255), nullable=True, comment="Currently executing workflow or task"
    )

    # Team coordination settings
    coordination_config = Column(
        JSONB, nullable=True, comment="Coordination-specific configuration and rules"
    )
    ...
    team_metadata = Column(
        JSONB, nullable=True, comment="Team-specific metadata and configuration"
    )
    ...
    agents = relationship(
        "Agent", secondary=team_agents, back_populates="teams", lazy="select"
    )
    ...
    def validate_communication_style(
    ...
    def update_execution_stats(
        self, success: bool = True, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    def create_hierarchical_team(
        cls, team_id: str, name: str, leader_id: str, members: List[str], **kwargs
    ) -> "Team":
    ...
    def create_collaborative_team(
        cls, team_id: str, name: str, members: List[str], **kwargs
    ) -> "Team":
