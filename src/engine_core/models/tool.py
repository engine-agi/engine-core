...
class Tool(SQLAlchemyBaseModel):
    ...
    name = Column(
        String(255), nullable=False, index=True, comment="Human-readable tool name"
    )
    ...
    version = Column(String(50), nullable=True, comment="Tool version information")
    ...
    authentication = Column(
        JSONB, nullable=True, comment="Authentication configuration and credentials"
    )

    # Tool capabilities and commands
    capabilities = Column(
        JSONB, nullable=True, comment="Tool capabilities and supported operations"
    )

    commands = Column(
        JSONB, nullable=True, comment="Available commands with parameters and mappings"
    )

    # Rate limiting and quotas
    rate_limits = Column(
        JSONB, nullable=True, comment="Rate limiting configuration and current usage"
    )

    # Tool validation and testing
    health_check = Column(
        JSONB, nullable=True, comment="Health check configuration and last results"
    )

    # Usage statistics
    usage_count = Column(
        Integer, default=0, nullable=False, comment="Total number of tool invocations"
    )
    ...
    last_used_at = Column(
        String(50), nullable=True, comment="Timestamp of last tool usage"
    )

    # Error tracking
    error_count = Column(
        Integer, default=0, nullable=False, comment="Number of tool errors"
    )

    last_error = Column(
        JSONB, nullable=True, comment="Details of last error encountered"
    )

    # Tool-specific metadata (avoiding conflict with SQLAlchemy's reserved 'metadata' attribute)
    tool_metadata = Column(
        JSONB, nullable=True, comment="Tool-specific metadata and configuration"
    )
    ...
    def update_health_check(self, status: str, details: Dict[str, Any]) -> None:
        ...
        health_check = {
            ...
            "previous_status": getattr(self, "health_check", {}).get("status")
            if getattr(self, "health_check")
            else None,
        }
    ...
    def record_usage(
        self, success: bool = True, error_details: Optional[Dict[str, Any]] = None
    ) -> None:
        ...
        return cls(
            ...
            capabilities={
                "repository_management": {
                    "description": "Create, read, update repositories"
                },
