...
class Protocol(SQLAlchemyBaseModel):
    ...
    name = Column(
        String(255), nullable=False, index=True, comment="Human-readable protocol name"
    )

    description = Column(
        Text, nullable=True, comment="Protocol purpose, usage patterns, and behavior"
    )
    ...
    validation_rules = Column(
        JSONB, nullable=True, comment="Validation rules for command usage and execution"
    )
    ...
    context_requirements = Column(
        JSONB, nullable=True, comment="Context requirements for protocol activation"
    )
    ...
    protocol_metadata = Column(
        JSONB, nullable=True, comment="Protocol-specific metadata and configuration"
    )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<Protocol(id='{self.id}', name='{self.name}', commands={command_count})>"
        )
    ...
    def find_commands_by_semantic_meaning(self, meaning: str) -> List[Dict[str, Any]]:
        ...
        return [cmd for cmd in commands if cmd.get("semantic_meaning") == meaning]
    ...
    def get_next_commands(
        self, current_command: str, context: Dict[str, Any]
    ) -> List[str]:
...
CheckConstraint(Protocol.usage_count >= 0, name="ck_protocol_usage_count_non_negative")
