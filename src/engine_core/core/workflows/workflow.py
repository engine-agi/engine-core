...
class AgentVertexComputation(VertexComputation):
    ...
    async def compute(
        ...
        try:
            ...
            response = await self.agent.execute(agent_input)
            ...
            # Extract output data from agent response
            output_content = ""
            if response.messages:
                # Get the last message content as output
...
class AtomicVertexComputation(VertexComputation):
    ...
    async def compute(
        ...
        try:
            ...
            operation_input = self._prepare_operation_input(
...
class WorkflowEngine:
    ...
    def __init__(self):
        ...
        self.edges: Dict[str, List[str]] = defaultdict(
            list)  # vertex_id -> [dependent_vertex_ids]
    ...
    def get_execution_order(self) -> List[List[str]]:
        ...
        levels = []
        current_level = [vertex_id for vertex_id,
            ...
            for superstep, level in enumerate(execution_levels):
                ...
                for vertex_id, result in level_results.items():
                    ...
                    if result.status == VertexState.COMPLETED:
                        ...
                        raise RuntimeError(
                            f"Vertex {vertex_id} execution failed: {
                                result.error}")
    ...
    def _collect_final_outputs(
            self, all_results: Dict[str, VertexExecutionResult]) -> Dict[str, Any]:
...
# Import WorkflowBuilder for convenience
