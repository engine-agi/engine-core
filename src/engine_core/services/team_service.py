    ...
    async def execute_tasks(
        ...
        try:
            ...
            context = TeamExecutionContext(
                project_id=request.context.get(
                    'project_id') if request.context else None,
                user_id=request.context.get('user_id') if request.context else None,
                session_id=request.context.get(
                    'session_id') if request.context else None,
                workflow_id=request.workflow_id,
            ...
            try:
                ...
                raise TeamExecutionError(
                    f"Team execution timed out after {timeout} seconds")
    ...
    async def _get_or_create_built_team(self, team_id: str) -> BuiltTeam:
        ...
        for member_data in team_data.get('members', []):
            ...
            try:
                ...
                logger.warning(
                    f"Could not load agent {agent_id} for team {team_id}: {str(e)}")
