...
class ProjectCreate(BaseModel):
    ...
    allowed_models: List[str] = Field(
        default_factory=list, description="List of allowed AI models"
    )
    max_agents: int = Field(
        default=50, ge=1, le=1000, description="Maximum number of agents"
    )
...
async def create_project(
    ...
    try:
        ...
        response = ProjectResponse(
            ...
            name=project["name"],
            description=project.get("description"),
            # Use first allowed model as default
            default_model=project["allowed_models"][0]
            if project["allowed_models"]
