...
class AgentBuilder:
    ...
    def with_stack(self, stack: List[str]) -> 'AgentBuilder':
        ...
        if not stack or not isinstance(stack, list) or not all(isinstance(s, str) for s in stack):
    ...
    def validate(self) -> bool:
        ...
        if self.config.get('model'):
            ...
            if not any(model in self.config['model'] for model in supported_models):
                self._validation_errors.append(f"Model '{self.config['model']}' is not supported")
        ...
        if self.config.get('stack'):
            ...
            supported_technologies = [
                ...
                'spring', 'dotnet', 'tensorflow', 'pytorch', 'pandas', 'numpy', 'scikit-learn',
                'docker', 'kubernetes', 'aws', 'azure', 'gcp', 'linux', 'windows', 'macos',
                'git', 'github', 'gitlab', 'jenkins', 'mongodb', 'postgresql', 'mysql', 'redis',
                'graphql', 'rest', 'soap', 'webassembly', 'wasm'
            ]
            
            for tech in self.config['stack']:
                if tech not in supported_technologies:
                    self._validation_errors.append(f"Technology '{tech}' is not supported")
    ...
    def build(self) -> 'BuiltAgent':
        ...
        if not self.validate():
            raise ValueError(f"Agent validation failed: {', '.join(self._validation_errors)}")
    ...
    def build_async(self) -> 'BuiltAgent':
        ...
        # For now, same as build() - in real implementation would validate async resources
        return self.build()
