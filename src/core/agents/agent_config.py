...
class AgentConfig(BaseModel):
    ...
    id: str = Field(
        ..., description="Unique agent identifier", min_length=1, max_length=100
    )
    model: AgentModel = Field(..., description="AI model for agent execution")
    stack: List[str] = Field(
        ..., description="Technology stack (programming languages, frameworks)"
    )

    # Optional fields (progressive configuration)
    name: Optional[str] = Field(
        None, description="Human-readable agent name", max_length=200
    )
    speciality: Optional[str] = Field(
        None, description="Agent's domain expertise", max_length=500
    )
    persona: Optional[str] = Field(
        None, description="Behavioral characteristics and personality", max_length=1000
    )
    tools: Optional[List[str]] = Field(
        None, description="Tool IDs available to the agent"
    )
    protocol_id: Optional[str] = Field(
        None, description="Associated protocol identifier"
    )
    workflow_id: Optional[str] = Field(
        None, description="Associated workflow identifier"
    )
    book_id: Optional[str] = Field(
        None, description="Associated memory book identifier"
    )
...
class AgentExecutionConfig(BaseModel):
    ...
    max_execution_time: int = Field(
        default=300, description="Maximum execution time in seconds", ge=1, le=3600
    )
    max_memory_mb: int = Field(
        default=512, description="Maximum memory usage in MB", ge=64, le=4096
    )
    max_concurrent_tasks: int = Field(
        default=5, description="Maximum concurrent tasks", ge=1, le=20
    )
...
class AgentTemplate(BaseModel):
    ...
    category: str = Field(
        ..., description="Template category (e.g., 'development', 'analysis')"
    )

    base_config: AgentConfig = Field(..., description="Base agent configuration")
    required_customizations: List[str] = Field(
        default_factory=list, description="Fields that must be customized"
    )

    tags: List[str] = Field(
        default_factory=list, description="Template tags for search"
    )
