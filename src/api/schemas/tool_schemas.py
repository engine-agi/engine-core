...
from .enums import ToolType, AuthenticationType
...
class APIToolConfigSchema(BaseModel):
    ...
    default_headers: Dict[str, str] = Field(default_factory=dict, description="Default headers")
    rate_limit_config: Dict[str, Any] = Field(default_factory=dict, description="Rate limit configuration")
    timeout_seconds: int = Field(default=30, description="Timeout in seconds")
...
class ToolUpdateSchema(BaseModel):
    ...
    configuration: Optional[Dict[str, Any]] = Field(default=None, description="Tool configuration")
    health_check_enabled: Optional[bool] = Field(default=None, description="Health check enabled")
