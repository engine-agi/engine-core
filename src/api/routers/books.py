...
class BookCreate(BaseModel):
    ...
    id: str = Field(..., min_length=1, max_length=50, description="Unique book identifier")
    name: str = Field(..., min_length=1, max_length=100, description="Human-readable book name")
    cover: BookCover = Field(..., description="Book cover metadata")
    enable_semantic_search: bool = Field(default=True, description="Enable semantic search indexing")
    enable_context_tracking: bool = Field(default=True, description="Enable context tracking")
    agent_ids: List[str] = Field(default_factory=list, description="Agents with access to this book")
