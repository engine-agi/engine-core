"""
Agents API Router
Handles agent lifecycle management within projects including CRUD operations and task execution.

This router provides endpoints for managing agents within the context of specific projects.
...
from ...shared_types.engine_types import EngineError, AgentStatus
from ...services.agent_service import AgentService, AgentCreateRequest, AgentUpdateRequest
...
class AgentCreate(BaseModel):
    ...
    id: str = Field(..., min_length=1, max_length=50, description="Unique agent identifier")
    name: Optional[str] = Field(None, max_length=100, description="Human-readable agent name")
    model: str = Field(..., description="AI model to use for this agent")
    speciality: Optional[str] = Field(None, max_length=200, description="Agent's area of expertise")
    persona: Optional[str] = Field(None, max_length=500, description="Agent's personality and behavior")
    stack: List[str] = Field(default_factory=list, description="Technology stack expertise")
    tools: List[str] = Field(default_factory=list, description="Available tools and integrations")
    protocol_id: Optional[str] = Field(None, description="Protocol for behavior patterns")
...
class AgentUpdate(BaseModel):
    ...
    tools: Optional[List[str]] = Field(None, description="Available tools and integrations")
    protocol_id: Optional[str] = Field(None, description="Protocol for behavior patterns")
    workflow_id: Optional[str] = Field(None, description="Associated workflow")
    book_id: Optional[str] = Field(None, description="Knowledge base reference")
...
class TaskExecution(BaseModel):
    ...
    context: Optional[str] = Field(None, max_length=2000, description="Additional context for the task")
    ...
    try:
        ...
        agents = [
            AgentSummary(
                ...
                name=agent.get("name"),
                model=agent["model"],
                speciality=agent.get("specialty"),  # Note: using "specialty" not "speciality"
                stack=agent.get("stack", []),
                status=agent.get("status", "unknown"),
                created_at=datetime.fromisoformat(agent["created_at"]) if isinstance(agent.get("created_at"), str) else agent.get("created_at", datetime.utcnow())
            )
...
async def create_agent(
    ...
    try:
        ...
        response = AgentResponse(
            ...
            status=agent.get("status", "unknown"),
            created_at=datetime.fromisoformat(agent["created_at"]) if isinstance(agent.get("created_at"), str) else agent.get("created_at", datetime.utcnow()),
            updated_at=datetime.fromisoformat(agent["updated_at"]) if agent.get("updated_at") and isinstance(agent.get("updated_at"), str) else None
        )
...
async def update_agent(
    ...
    try:
        ...
        if agent_data.model and agent_data.model not in project.get("allowed_models", []):
            raise HTTPException(
                status_code=400,
                detail=f"Model '{agent_data.model}' not allowed in this project"
            )
        ...
        response = AgentResponse(
            ...
            speciality=updated_agent.get("specialty"),  # Note: using "specialty" not "speciality"
            ...
            created_at=datetime.fromisoformat(updated_agent["created_at"]) if isinstance(updated_agent.get("created_at"), str) else updated_agent.get("created_at", datetime.utcnow()),
            updated_at=datetime.fromisoformat(updated_agent["updated_at"]) if updated_agent.get("updated_at") and isinstance(updated_agent.get("updated_at"), str) else None
        )
...
async def execute_task(
    ...
    try:
        ...
        execution = {
            ...
            "estimated_completion": datetime.utcnow() + timedelta(seconds=execution_data.timeout_seconds)
        }
